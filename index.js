...
let laMetricTest = (indexNumber) => {
  if (config.LaMetric[indexNumber] != null) {
    // TODO further inspection
    // const bla = async () => {
    //   await fetchLametric(config.LaMetric[indexNumber]);
              ...
              .then((PiHoleTopItemsData) => {
                fetch(
                  ...
                  .then((PiHoleRecentBlockedData) => {
                    availableLaMetrics.forEach((config) => {
                      let updateIndex = 1;
                      let updateSpinner = ora(
                        `Connecting to LaMetric @ ${config.IP}...`
                      ).start();
                      fetchWithAuth(
                        `http://${config.IP}:8080/api/v2/device/apps/com.lametric.58091f88c1c019c8266ccb2ea82e311d`,
                        createBasicAuth(config)
                      )
                        .then((LaMetricDeviceInfo) => {
                          fetchWithAuth(
                            `http://${config.IP}:8080/api/v2/device`,
                            createBasicAuth(config)
                          ).then((LaMetricDeviceInfo2) => {
                            updateSpinner.text = `Sending update for "${LaMetricDeviceInfo2.body.name}" @ ${config.IP} to the server...`;
                            ...
                            fetch(
                              ...
                              .then(() => {
                                ...
                                updateSpinner.succeed(
                                  `Sent update for "${LaMetricDeviceInfo2.body.name}" @ ${config.IP} to the server!`
                                );
                              })
                        ...
                        .catch((err) => {
                          ...
                          if (
                            ...
                            if (err.statusCode == 401) {
                              updateSpinner.fail(
                                `Update failed to send for LaMetric @ ${config.IP}. Auth invalid.`
                              );
                            } else if (err.statusCode == 404) {
                              updateSpinner.fail(
                                `Update failed to send for LaMetric @ ${config.IP}. Pi-Hole Status app not installed on the LaMetric.`
                              );
                            } else {
                              updateSpinner.fail(
                                `Update failed to send for LaMetric @ ${config.IP}. LaMetric does not seem to linked to this IP.`
                              );
                            }
                          } else {
                            updateSpinner.fail(
...
// TODO MMI
async function fetchLametric(config) {
  ...
  fetchWithAuth(
    ...
    .then((LaMetricDeviceInfo) => {
      ...
      fetchWithAuth(
        ...
        spinner.succeed(
          `Connected to "${LaMetricDeviceInfo2.body.name}" @ ${config.IP} running OS v${LaMetricDeviceInfo2.body.os_version} & Pi-Hole Status v${LaMetricDeviceInfo.body.version}! (${LaMetricDeviceInfo2.body.serial_number})`
        );
        availableLaMetrics.push(config);
        return new Promise();
      });
    })
    .catch((err) => {
      ...
      if (err.statusCode != null && err.body.errors != null) {
        if (err.statusCode == 401) {
          spinner.fail(
            `Connection to LaMetric @ ${config.IP} Failed. Auth invalid.`
          );
        } else if (err.statusCode == 404) {
          spinner.fail(
            `Connection to LaMetric @ ${config.IP} Failed. Pi-Hole Status app not installed on the LaMetric.`
          );
        } else {
          spinner.fail(
            `Connection to LaMetric @ ${config.IP} Failed. LaMetric does not seem to linked to this IP.`
          );
        }
      } else {
        spinner.fail(
