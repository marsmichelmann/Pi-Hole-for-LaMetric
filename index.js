          ...
          if (successfulLaMetricConnections > 0) {
            ...
            snek
              ...
              .then((updateCheckResult) => {
                if (
                  JSON.parse(updateCheckResult.body).version == config.version
                ) {
                  spinner.info("Up to date.");
                } else {
                  spinner.warn(
                    `New update available! (${
                      JSON.parse(updateCheckResult.body).version
                    })`
                  );
                }
                let updateLaMetric = () => {
                                    ...
                                    snek
                                      ...
                                      .then((LaMetricDeviceInfo2) => {
                                        ...
                                        let topQueryArray = Object.values(
                                          PiHoleTopItemsData.body.top_queries
                                        );
                                        let topBlockedQueryArray = Object.values(
                                          PiHoleTopItemsData.body.top_ads
                                        );
                                        snek
                                          .post(
